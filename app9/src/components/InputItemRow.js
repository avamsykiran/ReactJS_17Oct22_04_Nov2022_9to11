...
import { addItemActionThunkCreater, updateItemActionThunkCreater, unMarkItemEditableActionCreater } from '../state/InventoryReducer';

const InputItemRow = ({ inputItem }) => {
    ...
    const saveItem = e => {
        if (item.editable) {
            dispatch(updateItemActionThunkCreater({ ...item, editable: undefined }));
        } else {
            dispatch(addItemActionThunkCreater({ ...item }));
            ...
            {item.editable ?
                <div className='col-sm-2'>
                    ...
                    <button className='btn btn-sm btn-danger'
                        onClick={e => dispatch(unMarkItemEditableActionCreater(item.id))}>CANCEL</button>
                </div> :
                <div className='col-sm-2'>
