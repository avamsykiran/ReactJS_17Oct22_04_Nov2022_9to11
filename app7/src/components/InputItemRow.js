...
import { addItemActionCreater, updateItemActionCreater, unMarkItemEditableActionCreater } from '../state/InventoryReducer';
...
const InputItemRow = ({ inputItem }) => {
    ...
    const saveItem = e => {
        if (item.editable) {
            dispatch(updateItemActionCreater({ ...item, editable: undefined }));
        } else {
            dispatch(addItemActionCreater({ ...item }));
